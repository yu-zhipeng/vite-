...
export async function traverseHtml(
  ...
  // lazy load compiler
  // 之前用的是@vue/compiler-dom
  // 通过parse5的parse方法将html转换成 AST
  ...
  // 调用traverseNodes方法对每层 AST 调用传入的visitor，
  // 这个visitor（访问器）就是上面devHtmlHook传给traverseHtml函数的回调。
  // 也就是说每访问一层 AST 就会执行一次这个回调。
...
// applyHtmlTransforms就是按顺序调用传入的函数
// 如果传入的函数返回值中有tags属性，是一个数组；
// 遍历这个数组，根据injectTo属性分类，将这些tag分别加入bodyTags、bodyPrependTags、headTags和headPrependTags中。
// 所有函数执行完后，再调用injectToHead、injectToBody插入html中，最后返回转换后的html
