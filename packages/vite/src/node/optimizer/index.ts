...
export function loadCachedDepOptimizationMetadata(
  ...
  if (!force) {
    ...
    // 调用getDepHash，这个函数是读取目录下的package-lock.json的内容，然后将文件内容进行hash得到一个hash值
    // 然后两个hash进行对比是否相等
    // 也就是说，预编译就是看node_modules的包有没有变化，
    // 如果不相等，会调用scanImports去扫，在scanImports中，得到入口文件后，对入口文件进行了解析
...
export async function discoverProjectDependencies(
  ...
  // 扫描导入信息
  // 依赖扫描的核心思路就是先将代码解析成 AST 抽象语法树，然后找到 Import 节点
  // 这就涉及到了树的深度遍历，什么时候停止深入扫描，如何处理叶子节点都有一定的规则
  // 尤其是 HTML、Vue 等模块，处理过程相对比较复杂
  // vite并没有自己实现逻辑，而是使用esbuild的能力来进行深度遍历（本质上打包过程也是个深度遍历模块的过程）
  // 依赖esbuild的插件能力实现解析逻辑，找到Import 节点，以及对应的依赖位置
  // 1,叶子节点的处理
	// 		esbuild 可以对每个模块（叶子节点）进行解析和加载可以通过插件对这两个过程进行扩展，加入一些特殊的逻辑例如将 html 在加载过程中转换为 js
	// 2,不深入处理模块
	// 		esbuild 可以在解析过程，指定当前解析的模块为 external，则esbuild 不再深入解析和加载该模块。
	// 3,深入遍历模块
...
// 最终将资源写入.vite/deps中，依赖描述 optimized 以及 browserHash 等写入到 .vite/deps/metadata.json 中
export async function runOptimizeDeps(
  ...
  // 获取缓存路径 C:/xxx/GitHub/vite/playground/alias/node_modules/.vite/deps
  const depsCacheDir = getDepsCacheDir(resolvedConfig, ssr)
  // 缓存目录  C:/xxx/GitHub/vite/playground/alias/node_modules/.vite/deps_temp
...
export async function extractExportsData(
  ...
  if (optimizeDeps.extensions?.some((ext) => filePath.endsWith(ext))) {
    ...
    // 对于 esbuild build 的执行过程，入口文件为每一个 deps
    const result = await build({
      ...esbuildOptions,
...
export function getDepHash(config: ResolvedConfig, ssr: boolean): string {
  ...
  // 从 config.root 也就是项目的根目录下依次的寻找 package-lock.json、yarn.lock、pnpm-lock.yaml 文件，存在其中一个则读取文件的内容
