...
async function createDepsOptimizer(
  ...
  // 加载缓存的预编译元数据
  // 获取node_modules/.vite/_metadata.json中的元信息
  ...
  if (!cachedMetadata) {
    ...
    if (!isBuild) {
      ...
      depsOptimizer.scanProcessing = new Promise((resolve) => {
        ...
        // 在执行依赖扫描之前，先执行server listen，这里进行setTimeout
        setTimeout(async () => {
          try {
            debug(colors.green(`scanning for dependencies...`))
            // 先扫描出项目代码中的裸依赖 bare deps
            ...
            if (!isBuild) {
              ...
              // 对于dev，我们运行扫描程序，并在后台运行第一次优化，但我们要等到爬行结束后才决定是将结果发送到浏览器，还是需要执行另一个优化步骤
              // 到这里，第一次运行时，还没有生产缓存.vite文件
              // 后面要做的就是将依赖信息保存到.vite下。最后，执行writeFile，再将相关信息保存到_metadata.json
